...
window.onload = () => {
    ...
    class Avatar {
        ...
        preRender() {
            ...
            if (this.state.target.current && this.state.target.engaged) {
              let x1 = this.trans.offsetX-(this.map.centerX || $CURRENT_MAP.centerX);
              let y1 = this.trans.offsetY-(this.map.centerY || $CURRENT_MAP.centerY);
 
              let x2 = this.state.target.current.trans.offsetX-(this.map.centerX || $CURRENT_MAP.centerX);
              let y2 = this.state.target.current.trans.offsetY-(this.map.centerY || $CURRENT_MAP.centerY);
 
              this.trans.rotation = Math.atan2(y2-y1,x2-x1)-1.5708;
            } 
    ...
    let _MAP_ = new _Map_(200, 200).init();
    $CURRENT_MAP = _MAP_;
    _MAP_.showGeometry();

    let a = new Avatar("Borjij",-50,0);
    let b = new Avatar("efjifje",50,0);
    _MAP_.link(a);
    _MAP_.link(b);
    ...
    a.state.draw = true;
    a.state.fire = false;      
    a.state.target.engaged = true;
    a.state.target.current = b;
