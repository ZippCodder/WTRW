...
window.onload = () => {
 ...
 return false;
        ...
        gotoAvatar() {
         this.findPathTo(this.map.centerX,this.map.centerY);
        }
            ...
            if (obj.render || obj.hasCluster || obj.isValid) {
                ...
                if (obj.obstacle && obj.name !== "avatar") {
                 for (let i of obj.segments) { 
                  this.GRAPH.evalObstacle((i[0]+obj.trans.offsetX),(-(i[1])+obj.trans.offsetY),i[2],i[3]);
                 }
                }
   ...
   // let _MAP_ = new _Map_(780,280).init();
    let _MAP_ = new _Map_(150, 150).init();
   // _MAP_.link(new VisibleBarrier(30,30,10,10));
   // _MAP_.link(new VisibleBarrier(-30,-30,10,10));
   // _MAP_.link(new VisibleBarrier(30,10,10,10));
    $CURRENT_MAP = _MAP_;

    _MAP_.link(new Avatar("Roger Hawkinson",-25,0));
    _MAP_.showGeometry();
