  ...
  const enemySpawnLoop = new LoopAnimation(function() {
      if ($MAP.SUB_MAPS[0].avatarCount < $MAX_ENEMIES) {
          ...
          let a = new Bot(((!ally) ? "T R I N I T Y" : getName()), (x + 5) - $MAP.SUB_MAPS[0].centerX, (y - 5) - $MAP.SUB_MAPS[0].centerY);
          ...
          if (Math.random() < 0.5) {
              ...
              weapon = [new DX_9, new NXR_44_MAG, new FURS_55, new CombatKnife, new ClassicShotgun, new RobberShotgun][random(5)];
              a.state.killValue = 176;
          } else {
              weapon = [new NOSS_7, new X6_91, new USP_45, new HeavyShotgun][random(4)];
              a.state.killValue = 208;
          }

          if (ally) a.addItem(weapon);
          ...
          if (ally) {
              ...
              $AVATAR.state.targetId = genObjectId();

              let gun = new StubbyShotgun;
              gun.bullets = 1000;
              a.state.vitals.health = 10000;
              a.addItem(new StubbyShotgun);
              a.equipItem(0);
              a.follow($AVATAR.id);
              a.killTarget([$AVATAR.state.targetId], true, true);
              ally = true;
          }
      }
  }, window, 10);
  ...
  const timeUpdateLoop = new LoopAnimation(function() {
      ...
      let hour = Math.floor(globalTime / 60),
          minute = globalTime - (Math.floor(globalTime / 60) * 60);
